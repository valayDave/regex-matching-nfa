...
/**
 * <summary>Take a bunch of Filename Vectors and extract the contents of the
 * files within those files.</summary> <param name="fileNames"> A Constant
 * Vector Reference Passed to the function. Constant because it will not
 * change.</param> <returns> Vector Of Strings that have the contents of the
 * file</returns>
 */
vector<string> readFiles(const vector<string>& fileNames) {
  // Use Vectors to Create an array of File Contents
  vector<string> files;
  // TODO : Might Change the string postion from here to inside the loop while
  ...
  for (ptr = fileNames.begin(); ptr < fileNames.end(); ptr++) {
    string fileContent = "";
    // As Reference is passed to the Function a pointer is used to reference of
    // the actual index in the vector
    ...
    if (myfile.is_open()) {
      ...
      myfile.close();
      // Write the Content of the File to the Vector and pass the Vector back to
      // the
...
/**
 * <summary>Creates a Vector of strings to match for the Regular Expression
 * Provided</summary> <param name='expression'>Regualar expression of the type
 * "a+be*" or "(ab)+(c)"</param>
 */
...
int main(int argc, char* argv[]) {
  ...
  // Vector is chosen. This is Inefficient. I can use strings Instead of Vectors.
  ...
  // TODO :Find the Largest word in a string. --> This is done so that the
  // extent to which the * operator's evaluation needs to take place.

  // TODO :TURN Regex Ex Into an array of strings that need to be evaluated.

  // TODO:Match The File Contents Strings with the array of strings that were
  // found.
